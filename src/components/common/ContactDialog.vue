            ...
            <form @submit.prevent="submit" class="grid gap-8">
              ...
              <div class="input-group z-20 w-full" v-if="contact.topics.length > 1">
                <Popper placement="bottom-start" offsetDistance="1" :show="showPopper" class="w-full">
                  <button type="button" @click="showPopper = !showPopper" class="select surface-overlay w-full text-left">
                    {{ !!topic ? topic : "Select" }}
                  </button>
...
const submit = () => {
  ...
  const formData = {
    access_key: "ac12f457-93c4-41d4-b974-cabd70edfc51", // Replace with your actual access key
    subject: "Dice Wizard has a new message",
    name: form.name,
    email: form.email,
  ...
  fetch("https://api.web3forms.com/submit", {
    ...
    .then(async (response) => {
      ...
      if (response.ok) {
        toast.success(data.message || "Thank you for your submission.");
